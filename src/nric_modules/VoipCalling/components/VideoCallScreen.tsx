...
const VideoCallScreen: React.FC<VideoCallScreenProps> = ({
  ...
  const switchCamera = async () => {
    ...
    try {
      ...
      currentAudioTracks.forEach(track => newStream.addTrack(track));
      ...
      try {
        switch (data.type) {
          ...
          case 'candidate':
            ...
            break;
          case 'mute_status':
            const wasRemoteMuted = isRemoteMuted;
            setIsRemoteMuted(data.isMuted);
            setShowUnmuteRequest(false);

            if (callConnected && wasRemoteMuted !== data.isMuted) {
              if (data.isMuted) {
    Vibration.vibrate(200); // Vibrate for 200ms when remote mutes
  }

              Alert.alert(
                  data.isMuted ? 'Microphone Off' : 'Microphone On',
                  `The other user has ${data.isMuted ? 'muted' : 'unmuted'} their microphone`
                    );
            }
            break;
  ...
  <View style={styles.remoteMuteIndicator}>
    ...
    <Text style={styles.remoteMuteText}>Other user muted their microphone</Text>
    {!isCaller && (
      <TouchableOpacity
